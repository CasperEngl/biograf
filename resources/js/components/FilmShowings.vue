<template>
  <div class="row">
    <div class="col w-1/3 mb-6" v-for="film in filteredFilms" :key="film.id">
      <a :href="route('film.show', {
        slug: film.slug,
      })" class="overflow-hidden relative block">
        <figure class="relative aspect-ratio-16/9 mb-2 block">
          <img :src="film.backdrop[0]" onerror="this.src = '/img/placeholder/backdrop-small.png'" :alt="film.title" class="absolute">
        </figure>
      ...
      <p class="my-4" v-else>Ingen visninger på denne dato</p>
      <a :href="route('film.show', {
        slug: film.slug,
      })" class="my-1 w-full btn btn-ghost border-gray-500">Læs mere</a>
      <a :href="route('showing.days', {
        slug: film.slug
      })" class="my-1 w-full btn btn-ghost border-gray-500">Se alle dage</a>
    </div>
  </div>
</template>
